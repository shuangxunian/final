...
const makeSureUpload = async function() {
  const { data } = await axios.post('http://localhost:3000/text1/addList', {
    list: fileList.value
  })
  if (data.code === 2) {
    ElMessage({
...
const goAddText = async () => {
  ...
  if (textForm.value.pinyin === '') return ElMessage.error('请输入识字规则')
  const { data } = await axios.post('http://localhost:3000/text1/add', {
    ...textForm.value
  })
  if (data.code === 2) {
    ElMessage({
...
const getTextList = async () => {
  const { data } = await axios.post('http://localhost:3000/text1/allData', {})
  if (data.code === 2) {
...
const makeSureDel = async (row) => {
  const { data } = await axios.post('http://localhost:3000/text1/del', {
    id: row.id
  })
  if (data.code === 2) {
    ElMessage({
...
const goEditText = async () => {
  if (textForm.value.china === '') return ElMessage.error('请输入中文词语')
  if (textForm.value.pinyin === '') return ElMessage.error('请输入识字规则')
  const { data } = await axios.post('http://localhost:3000/text1/edit', {
    ...textForm.value
  })
  if (data.code === 2) {
    ElMessage({
  ...
  <div class="data-list">
    ...
    <div class="body">
      <el-table :data="tableData" border height="600">
        ...
        <el-table-column prop="pinyin" label="识字规则" />
