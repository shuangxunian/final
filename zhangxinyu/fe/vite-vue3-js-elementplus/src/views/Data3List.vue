...
const makeSureUpload = async function() {
  const { data } = await axios.post('http://localhost:3000/text3/addList', {
    list: fileList.value
  })
  if (data.code === 2) {
    ElMessage({
...
const goAddText = async () => {
  ...
  if (textForm.value.pinyin === '') return ElMessage.error('请输入词语句子')
  const { data } = await axios.post('http://localhost:3000/text3/add', {
    ...textForm.value
  })
  if (data.code === 2) {
    ElMessage({
...
const getTextList = async () => {
  const { data } = await axios.post('http://localhost:3000/text3/allData', {})
  if (data.code === 2) {
...
const makeSureDel = async (row) => {
  const { data } = await axios.post('http://localhost:3000/text3/del', {
    id: row.id
  })
  if (data.code === 2) {
    ElMessage({
...
const goEditText = async () => {
  if (textForm.value.china === '') return ElMessage.error('请输入中文词语')
  if (textForm.value.pinyin === '') return ElMessage.error('请输入词语句子')
  const { data } = await axios.post('http://localhost:3000/text3/edit', {
    ...textForm.value
  })
  if (data.code === 2) {
    ElMessage({
  ...
  <div class="data-list">
    <div class="header">
      ...
      <div class="right">
        <el-upload
          ...
          :limit="1"
        >
          <el-button type="primary">导入词句</el-button>
        </el-upload>
      </div>
    </div>
    <div class="body">
    ...
    <el-dialog v-model="textDialog" title="新建词语" width="500" @close="clearForm">
      <el-form :model="textForm">
        ...
        <el-form-item label="词语句子" :label-width="formLabelWidth">
