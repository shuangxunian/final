...
const goAddText = async () => {
  ...
  if (textForm.value.pinyin === '') return ElMessage.error('请输入演示视频链接')
  const { data } = await axios.post('http://localhost:3000/text4/add', {
    ...textForm.value
  })
  if (data.code === 2) {
    ElMessage({
...
const makeSureDel = async (row) => {
  const { data } = await axios.post('http://localhost:3000/text4/del', {
    id: row.id
  })
  if (data.code === 2) {
    ElMessage({
...
const goEditText = async () => {
  if (textForm.value.china === '') return ElMessage.error('请输入中文词语')
  if (textForm.value.pinyin === '') return ElMessage.error('请输入演示视频链接')
  const { data } = await axios.post('http://localhost:3000/text4/edit', {
    ...textForm.value
  })
  if (data.code === 2) {
    ElMessage({
  ...
  <div class="data-list">
    ...
    <div class="body">
      <el-table :data="tableData" border height="600">
        ...
        <el-table-column prop="pinyin" label="演示视频">
          <template #default="scope">
    ...
    <el-dialog v-model="textDialog" title="新建词语" width="500" @close="clearForm">
      <el-form :model="textForm">
        ...
        <el-form-item label="演示视频链接" :label-width="formLabelWidth">
