...
user.post('/edit', async (req, res) => {
  const { body } = req
  let sql = `update user_info set username='${body.username}', roleType ='${body.roleType}' where id='${body.id}'`
  console.log(sql)
  const database = new DataBase()
  await database.getSqlData(sql)
  res.send({
...
user.post('/add', async (req, res) => {
  ...
  if (info.length) {
    ...
    sql = `insert into user_info (id,username,password,roleType) values ('${body.id}','${body.username}','${body.password}','${body.roleType}')`
    const addDatabase = new DataBase()
