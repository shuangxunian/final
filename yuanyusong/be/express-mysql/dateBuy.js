...
const dateBuy = express.Router()

dateBuy.post('/allData', async (req, res) => {
  let sql = 'select * from date_buy_list'
...
dateBuy.post('/add', async (req, res) => {
  const { body } = req
  let sql =  `insert into date_buy_list (id,foodid,foodfrom,imgUrl,date) values ('${new Date().getTime()}','${body.foodid}','${body.foodfrom}','${body.imgUrl}','${body.date}')`
  const database = new DataBase()
...
dateBuy.post('/del', async (req, res) => {
  const { body } = req
...
dateBuy.post('/fix', async (req, res) => {
  const { body } = req
...
dateBuy.post('/fixPassword', async (req, res) => {
  const { body } = req
...
dateBuy.post('/edit', async (req, res) => {
  const { body } = req
  let sql = `select * from food_list where name='${body.name}'`
  const database = new DataBase()
  const info = await database.getSqlData(sql)
...
dateBuy.post('/addList', async (req, res) => {
  const { body } = req
