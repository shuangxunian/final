...
const starClass = async function(row, flag) {
  // console.log(row)
  const { data } = await axios.post('http://localhost:3000/selectClass/star', {
    userid: userid.value,
    classid: row.id,
    flag
  })
...
const getSelectClassList = async function() {
  const { data } = await axios.post('http://localhost:3000/selectClass/mylist',{
    userid: userid.value
  })
  if (data.code === 2) {
  ...
  <div class="admin-users">
    ...
    <el-dialog v-model="addPPTDialog" title="新建课程" width="500" @close="refreshForm">
      <el-form :model="pptForm">
        ...
        <el-form-item label="课件文档" :label-width="formLabelWidth">
          <el-upload
              class=""
              action=""
